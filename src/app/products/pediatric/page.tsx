...
export default function PediatricPage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'pediatric');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'pediatric', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const pediatricProducts = await loadCategoryProducts('pediatric');
        setAllProducts(pediatricProducts);
      } catch (error) {
        console.error('Error loading pediatric products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container pediatric-page">
      {/* Hero Section */}
      <section className="pediatric-hero" aria-label="Pediatric hero banner">
        <div className="pediatric-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Pediatric Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality pediatric formulations designed for children.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search pediatric products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'pediatric', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
