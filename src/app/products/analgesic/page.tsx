...
export default function AnalgesicPage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'analgesic');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'analgesic', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const analgesicProducts = await loadCategoryProducts('analgesic');
        setAllProducts(analgesicProducts);
      } catch (error) {
        console.error('Error loading analgesic products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container analgesic-page">
      {/* Hero Section */}
      <section className="analgesic-hero" aria-label="Analgesic hero banner">
        <div className="analgesic-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Analgesic Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality analgesic formulations.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search analgesic products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'analgesic', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
