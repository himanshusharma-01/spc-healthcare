...
export default function OrthopedicPage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'orthopedic');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'orthopedic', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const orthopedicProducts = await loadCategoryProducts('orthopedic');
        setAllProducts(orthopedicProducts);
      } catch (error) {
        console.error('Error loading orthopedic products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container orthopedic-page">
      {/* Hero Section */}
      <section className="orthopedic-hero" aria-label="Orthopedic hero banner">
        <div className="orthopedic-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Orthopedic Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality orthopedic formulations.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search orthopedic products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'orthopedic', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
