...
export default function AntibioticPage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'antibiotic');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'antibiotic', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const antibioticProducts = await loadCategoryProducts('antibiotic');
        setAllProducts(antibioticProducts);
      } catch (error) {
        console.error('Error loading antibiotic products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container antibiotic-page">
      {/* Hero Section */}
      <section className="antibiotic-hero" aria-label="Antibiotic hero banner">
        <div className="antibiotic-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Antibiotic Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality antibiotic formulations.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search antibiotic products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'antibiotic', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
