...
export default function DermaPage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'derma');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'derma', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const dermaProducts = await loadCategoryProducts('derma');
        setAllProducts(dermaProducts);
      } catch (error) {
        console.error('Error loading derma products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container derma-page">
      {/* Hero Section */}
      <section className="derma-hero" aria-label="Derma hero banner">
        <div className="derma-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Derma Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality dermatological formulations.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search derma products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'derma', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
