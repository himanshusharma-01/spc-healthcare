...
export default function GynaePage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'gynae');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'gynae', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const gynaeProducts = await loadCategoryProducts('gynae');
        setAllProducts(gynaeProducts);
      } catch (error) {
        console.error('Error loading gynae products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container gynae-page">
      {/* Hero Section */}
      <section className="gynae-hero" aria-label="Gynae hero banner">
        <div className="gynae-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Gynae Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality gynecological formulations.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search gynae products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'gynae', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
