...
export default function GastroPage() {
  ...
  const subcategories = useMemo(() => {
    return extractSubcategories(allProducts, 'gastro');
  }, [allProducts]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeSubcategory !== 'all') {
      filtered = filterProductsBySubcategory(filtered, 'gastro', activeSubcategory);
    }
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        const gastroProducts = await loadCategoryProducts('gastro');
        setAllProducts(gastroProducts);
      } catch (error) {
        console.error('Error loading gastro products:', error);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="l3-container gastro-page">
      {/* Hero Section */}
      <section className="gastro-hero" aria-label="Gastro hero banner">
        <div className="gastro-hero-background" role="presentation"></div>
      </section>
      ...
      <section className="products-section">
        <div className="l3-container-inner">
          <div className="section-header">
            <h2 className="l3-section-title">Our Gastro Products</h2>
            <p className="l3-section-subtitle">
              Discover our range of high-quality gastroenterological formulations.
            </p>
          </div>
          ...
          <div className="category-search-container">
            <div className="category-search-box">
              ...
              <input
                type="text"
                placeholder="Search gastro products by name, description, or usage..."
            ...
            <div className="products-filter-container">
              <div className="filter-tabs">
                {subcategories.map(subcategory => {
                  const count = filterProductsBySubcategory(allProducts, 'gastro', subcategory.id).length;
                  return (
                    <button
                      key={subcategory.id}
